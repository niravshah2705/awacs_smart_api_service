...
func Products(Productdetail *[]*model.ProductDetails, clouse string) (err error) {
	err = db.DB["smartdb"].Limit(1000).Where(clouse).Order("ProductName").Find(&Productdetail).Error
	return
}
...
func ProductsAwacs(Productdetail *[]*model.AWACSProduct, selectquery string, clouse string) (err error) {
	err = db.DB["awacs"].Limit(5000).Select(selectquery).Where(clouse).Find(&Productdetail).Error
	return
}
